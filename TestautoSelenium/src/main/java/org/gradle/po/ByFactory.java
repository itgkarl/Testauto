...
public class ByFactory {
	public By xpath(String xpath) {
		try {
			...
			throw new AssertionError("Invalid xpath syntax: " + xpath);
		}
	...
	public By name(String name) {
		return css(String.format("[name='%s']", name));
	}
	...
	public By link(String locator) {
		return xpath(
				".//A[@href][@id='%1$s' or text()='%1$s' or @title='%1$s' or .//img[@alt='%1$s']]",
				locator);
	}
	...
	private static String fieldXPath(String base, String locator) {
		return String
				.format(" .//%2$s[./@id = '%1$s' or ./@name = '%1$s' or ./@value = '%1$s' or ./@placeholder = '%1$s' or ./@id = //label[contains(normalize-space(string(.)), '%1$s')]/@for]"
						+ "| .//label[contains(normalize-space(string(.)), '%1$s')]//%2$s"
						+ "| .//label[contains(normalize-space(string(.)), '%1$s')][@class='attach-previous']/preceding-sibling::%2$s",
						locator, base);
	}
	...
	public By option(String name) {
		return xpath(".//option[contains(normalize-space(string(.)), '%1$s') or @value='%1$s']",
				name);
	}
